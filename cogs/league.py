...
class League(commands.Cog):
    ...
    async def rank(self, ctx, name=None, region=None):
        ...
        if region == None:
            ...
            return
        embed = discord.Embed(
            title=name + "'s Ranks in " + region, color=0xa9152b)
        region = region.upper()
        ...
        if region == "Invalid Region":
            ...
            async with aiohttp.ClientSession() as session:
                ...
                if rankedrequest != []:
                    for x in range(len(rankedrequest)):
                        if rankedrequest[x]["queueType"] == "RANKED_SOLO_5x5":
                            message = ""
                            message += (rankedrequest[x]["tier"] + " " + rankedrequest[x]["rank"] + " " + str(
                                rankedrequest[x]["leaguePoints"]) + "LP ")
                            if "miniSeries" in rankedrequest[x]:
                                ...
                                for y in range(len(rankedrequest[x]["miniSeries"]["progress"])):
                                    if rankedrequest[x]["miniSeries"]["progress"][y] == 'W':
                                        ...
                                        message += (":x:")
                            embed.add_field(
                                name="Solo", value=message, inline=False)
                            ...
                            if "miniSeries" in rankedrequest[x]:
                                ...
                                for y in range(len(rankedrequest[x]["miniSeries"]["progress"])):
                                    if rankedrequest[x]["miniSeries"]["progress"][y] == 'W':
                                        ...
                                        message += (":x:")
                            embed.add_field(
                                name="Flex", value=message, inline=False)
                        else:
                            continue
                # if tftrequest != []:
                ...
                if rankedrequest == []:  # and tftrequest == []:
                    embed.add_field(name="Unranked", value=name +
                                    " is unranked in all queues")
    ...
    async def profile(self, ctx, name=None, region=None):
        ...
        region = region.upper()
        embed = discord.Embed(
            title=name + "'s Profile on " + region, color=0xa9152b)
        region = await utils.region_to_valid_region(region)
        async with aiohttp.ClientSession() as session:
            async with session.get("https://" + region + ".api.riotgames.com/lol/summoner/v4/summoners/by-name/" + name + "?api_key=" + leagueapikey) as response:
                if response.status != 200:
            ...
            async with session.get("https://" + region + ".api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/" + summonerid + "?api_key=" + leagueapikey) as response:
                if response.status != 200:
            ...
            await session.close()
        embed.add_field(name="Top 3 Masteries",
                        value=mastery_message, inline=False)
        await ctx.send(embed=embed)
        return
