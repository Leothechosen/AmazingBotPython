            ...
            async with session.get("https://esports-api.lolesports.com/persisted/gw/getStandings?hl=en-US&tournamentId=" + tournamentId, headers=headers) as response:
                ...
                for x in range(len(rankings)):
                    for y in range(len(rankings[x]["teams"])):
                        ...
                        teams_message += str(rankings[x]
                                             ["teams"][y]["name"]) + '\n'
                        records_message += str(rankings[x]["teams"][y]["record"]["wins"]) + "-" + str(
                            rankings[x]["teams"][y]["record"]["losses"]) + '\n'
                embed = discord.Embed(
                    title=tournament.upper() + " Standings", color=0xa9152b)
                embed.add_field(
                    name="Place", value=ordinal_message, inline=True)
                embed.add_field(name="Team", value=teams_message, inline=True)
                embed.add_field(
                    name="Record", value=records_message, inline=True)
            ...
            async with session.get("https://esports-api.lolesports.com/persisted/gw/getSchedule?hl=en-US&leagueId=" + leagueid, headers=headers) as response:
                ...
                for x in range(len(scheduled_matches)):
                    if scheduled_matches[x]["state"] == "unstarted":
                        if team is None:
                            starttime = re.sub(
                                '[T]', " ", scheduled_matches[x]["startTime"])
                            ...
                            if currenttime + timedelta(days=7) > starttime:
                                ...
                                if leaguename == "LCSA":
                                    teams1_message += "**" + scheduled_matches[x]["match"]["teams"][0]["name"][:-8] + " (" + str(
                                        scheduled_matches[x]["match"]["teams"][0]["record"]["wins"]) + "-" + str(scheduled_matches[x]["match"]["teams"][0]["record"]["losses"]) + ")" + "**\n"
                                    teams2_message += "**" + scheduled_matches[x]["match"]["teams"][1]["name"][:-8] + " (" + str(
                                        scheduled_matches[x]["match"]["teams"][1]["record"]["wins"]) + "-" + str(scheduled_matches[x]["match"]["teams"][1]["record"]["losses"]) + ")" + "**\n"
                                else:
                                    teams1_message += "**" + scheduled_matches[x]["match"]["teams"][0]["name"] + " (" + str(
                                        scheduled_matches[x]["match"]["teams"][0]["record"]["wins"]) + "-" + str(scheduled_matches[x]["match"]["teams"][0]["record"]["losses"]) + ")" + "**\n"
                                    teams2_message += "**" + scheduled_matches[x]["match"]["teams"][1]["name"] + " (" + str(
                                        scheduled_matches[x]["match"]["teams"][1]["record"]["wins"]) + "-" + str(scheduled_matches[x]["match"]["teams"][1]["record"]["losses"]) + ")" + "**\n"
                        else:
                            if next_4_matches != 4:
                                if scheduled_matches[x]["match"]["teams"][0]["code"] == team or scheduled_matches[x]["match"]["teams"][1]["code"] == team:
                                    ...
                                    if scheduled_matches[x]["match"]["teams"][0]["code"] == team:
                                        team_record = "(" + str(scheduled_matches[x]["match"]["teams"][0]["record"]["wins"]) + "-" + str(
                                            scheduled_matches[x]["match"]["teams"][0]["record"]["losses"]) + ")"
                                        opponent_message += '**' + scheduled_matches[x]["match"]["teams"][1]["name"] + " (" + str(
                                            scheduled_matches[x]["match"]["teams"][1]["record"]["wins"]) + "-" + str(scheduled_matches[x]["match"]["teams"][1]["record"]["losses"]) + ")**\n"
                                        thumbnail = scheduled_matches[x]["match"]["teams"][0]["image"]
                                    else:
                                        team_record = "(" + str(scheduled_matches[x]["match"]["teams"][1]["record"]["wins"]) + "-" + str(
                                            scheduled_matches[x]["match"]["teams"][1]["record"]["losses"]) + ")"
                                        opponent_message += '**' + scheduled_matches[x]["match"]["teams"][0]["name"] + " (" + str(
                                            scheduled_matches[x]["match"]["teams"][0]["record"]["wins"]) + "-" + str(scheduled_matches[x]["match"]["teams"][0]["record"]["losses"]) + ")**\n"
                ...
                if team == None:
                    embed = discord.Embed(
                        title=leaguename.upper() + " Schedule", color=0xa9152b)
                    embed.add_field(name="Time Remaining",
                                    value=schedule_message, inline=True)
                    embed.add_field(
                        name="Team 1", value=teams1_message, inline=True)
                    embed.add_field(
                        name="Team 2", value=teams2_message, inline=True)
                    embed.set_footer(text="Next 7 Days")
                else:
                    embed = discord.Embed(
                        title=team.upper() + " " + team_record, color=0xa9152b)
                    embed.add_field(name="Time Remaining",
                                    value=schedule_message, inline=True)
                    embed.add_field(name="Opponent",
                                    value=opponent_message, inline=True)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("https://esports-api.lolesports.com/persisted/gw/getTeams?hl=en-US&id=" + team.lower(), headers=headers) as response:
                ...
                if team_response["data"]["teams"] == []:
                    ...
                    return
                embed = discord.Embed(
                    title=team.upper() + " Roster", color=0xa9152b)
                ...
                for x in range(len(names)):
                    name_message += names[x]['firstName'] + " '" + \
                        names[x]['summonerName'] + "' " + \
                        names[x]['lastName'] + '\n'
                embed.add_field(name="Name", value=name_message, inline=True)
                embed.set_thumbnail(
                    url=team_response["data"]["teams"][0]["image"])
        await ctx.send(embed=embed)
