...
class eSports(commands.Cog):
            ...
            async with session.get(
                ...
                for x in range(len(rankings)):
                    for y in range(len(rankings[x]["teams"])):
                        ...
                        records_message += (
                            str(rankings[x]["teams"][y]["record"]["wins"])
                            + "-"
                            + str(rankings[x]["teams"][y]["record"]["losses"])
                            + "\n"
                        )
    ...
    async def schedule(self, ctx, league=None, team=None):
        ...
        if leaguename == "LPL":
            await ctx.send(
                "Unfortunately, LPL is on hiatus due to the Coronavirus. This means that there is no schedule for the time being."
            )
            return
                        ...
                        if team is None:
                            ...
                            if currenttime + timedelta(days=7) > starttime:
                                ...
                                if leaguename == "LCSA":
                                    teams1_message += (
                                        "**"
                                        + scheduled_matches[x]["match"]["teams"][0]["name"][:-8]
                                        + " ("
                                        + str(scheduled_matches[x]["match"]["teams"][0]["record"]["wins"])
                                        + "-"
                                        + str(scheduled_matches[x]["match"]["teams"][0]["record"]["losses"])
                                        + ")"
                                        + "**\n"
                                    )
                                    teams2_message += (
                                        "**"
                                        + scheduled_matches[x]["match"]["teams"][1]["name"][:-8]
                                        + " ("
                                        + str(scheduled_matches[x]["match"]["teams"][1]["record"]["wins"])
                                        + "-"
                                        + str(scheduled_matches[x]["match"]["teams"][1]["record"]["losses"])
                                        + ")"
                                        + "**\n"
                                    )
                                else:
                                    teams1_message += (
                                        "**"
                                        + scheduled_matches[x]["match"]["teams"][0]["name"]
                                        + " ("
                                        + str(scheduled_matches[x]["match"]["teams"][0]["record"]["wins"])
                                        + "-"
                                        + str(scheduled_matches[x]["match"]["teams"][0]["record"]["losses"])
                                    ...
                                    teams2_message += (
                                        "**"
                                        + scheduled_matches[x]["match"]["teams"][1]["name"]
                                        + " ("
                                        + str(scheduled_matches[x]["match"]["teams"][1]["record"]["wins"])
                                        + "-"
                                        + str(scheduled_matches[x]["match"]["teams"][1]["record"]["losses"])
                            ...
                            if next_4_matches != 4:
                                if (
                                    scheduled_matches[x]["match"]["teams"][0]["code"] == team
                                    or scheduled_matches[x]["match"]["teams"][1]["code"] == team
                                ):
                                    starttime = re.sub("[T]", " ", scheduled_matches[x]["startTime"])
                                    starttime = re.sub("[Z]", "", starttime)
                                    starttime = datetime.strptime(starttime, "%Y-%m-%d %H:%M:%S")
    ...
    async def team(self, ctx, team=None):
        ...
        async with aiohttp.ClientSession() as session:
            headers = {"x-api-key": "0TvQnueqKa5mxJntVWt0w4LpLfEkrV1Ta8rQBb9Z"}
            async with session.get(
                "https://esports-api.lolesports.com/persisted/gw/getTeams?hl=en-US&id=" + team.lower(), headers=headers
            ) as response:
                ...
                for x in range(len(names)):
                    name_message += (
                        names[x]["firstName"] + " '" + names[x]["summonerName"] + "' " + names[x]["lastName"] + "\n"
                    )
