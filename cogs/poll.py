...
class Poll(commands.Cog):
    ...
    async def poll(self, ctx, *, arguments=None):
        if arguments == None:
            await ctx.send('Usage: `-poll [Question], [Answer1], [Answer2], ..., [Answer9], [Time in Seconds (Max: 300)]`')
            return
        ...
        if remainingseconds >= 60:
            timeremaining = str(int(remainingseconds/60)) + \
                " minutes, " + str(remainingseconds % 60) + " seconds"
        else:
            timeremaining = str(remainingseconds % 60) + " seconds"
        embed.add_field(name="Time Remaining",
                        value=timeremaining, inline=False)
        ...
        while remainingseconds != "done":
            if remainingseconds <= 0:
                embed.set_field_at(1, name="Time Remaining",
                                   value="Poll has finished", inline=False)
                    ...
                    if max_reacts == 1:
                        ...
                        for x in range(len(reactors[max_reacts])):
                            if x != len(reactors[max_reacts]) - 1:
                                tie_message += arguments[reaction_list.index(
                                    reactors[max_reacts][x])+1] + '" and "'
                            else:
                                tie_message += arguments[reaction_list.index(
                                    reactors[max_reacts][x])+1] + '"'
                    embed.add_field(
                        name="Tie!", value=tie_message, inline=False)
                else:
                    embed.add_field(name="Winner", value=arguments[reaction_list.index(
                        reactors[max_reacts][0])+1] + " wins!", inline=False)
                for x in range(len(msg.reactions)):
                    if msg.reactions[x].emoji in reaction_list:
                        if msg.reactions[x].count == 2:
                            answers_vote += str(
                                msg.reactions[x].count-1) + " vote\n"
                        else:
                            answers_vote += str(
                                msg.reactions[x].count-1) + " votes\n"
                embed.insert_field_at(
                    1, name="Votes", value=answers_vote, inline=True)
                await discord.Message.edit(msg, embed=embed)
                ...
                if remainingseconds >= 60:
                    timeremaining = str(
                        int(remainingseconds/60)) + " minutes, " + str(remainingseconds % 60) + " seconds"
                else:
                    timeremaining = str(remainingseconds % 60) + " seconds"
                embed.set_field_at(1, name="Time Remaining",
                                   value=timeremaining, inline=False)
